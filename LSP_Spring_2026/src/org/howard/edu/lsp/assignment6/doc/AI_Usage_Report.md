...
Note that is is a continued chat conversation from the previous assignment so some of the below text is a repeat from assignment 5
...
> Actually let's completely scrap the test suite and revamp it according to the following requirements:  
>
>*  Use JUnit Version 5  
>*  Include assertions such as (but not limited to) assertEquals, assertTrue, assertFalse, assertThrows  
>*  toString() format must be exactly [1, 2, 3]  
>*  include both a normal case AND an edge case for each of the methods  
>*  make sure there are exception tests  

>Here are the required edge cases for each method:  
...
Here's a full summary of the test suite structure:  

| Method | Normal Case Test | Edge Case Test |  
|---|---|---|
| `add()` | New element added correctly | Duplicate ignored |  
| `remove()` | Present element removed | Absent value — no change |  
| `equals()` | Same elements, same order | Same elements, different order |  
| `union()` | Two overlapping sets | With empty set |  
| `intersect()` | Common elements returned | Disjoint sets → `[]` |  
| `diff()` | Elements in A not in B | Identical sets → `[]` |  
| `complement()` | Elements in B not in A | Disjoint sets → all of B |  
| `contains()` | Value present → `true` | Value absent → `false` |  
| `isEmpty()` | Non-empty → `false` | New set + cleared set → `true` |  
| `toString()` | Ascending order, correct format | Empty set → `[]` |  
| `largest()` | Max of multi-element set | Single element + empty → exception |  
| `smallest()` | Min of multi-element set | Single element + empty → exception |  

A few design notes worth highlighting:  
- Every test is fully independent thanks to `@BeforeEach`, so no test can corrupt the state of another.  
- `assertThrows` is used exclusively for the two exception cases (`largest()` and `smallest()` on an empty set), keeping exception testing separate from assertion testing.  
- The `toString()` format `[1, 2, 3]` is checked literally as a string in every relevant test, so any spacing or bracket deviation will be caught immediately.  
